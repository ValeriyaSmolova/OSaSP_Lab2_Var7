...
int scaning(char *dir_name, long min_size, long max_size);
void dublfiles();
...
int main(int argc, char *argv[]) {
	...
	long min = strtol(argv[2], &endptr, 10);
	long max = strtol(argv[3], &endptr1, 10);
...
int scaning(char *dir_name, long min_size, long max_size)
{
	...
	while((Data = readdir(StartDir)) != NULL)
	{
		...
		else if(Data->d_type == DT_DIR && (strcmp(".", Data->d_name) != 0) && (strcmp("..", Data->d_name) != 0))
		{
		...
		{		
			...
			for (int j = i + 1; j < path_file_count; j++)
			{
				...
				if(c1 == c2 && c1 == EOF)
				{
					...
					{
						dubl = 1;
						printf("There are the following dublicates:\n\n");
					}
					if(hasdubl == 0)
					{
						printf("\n%s", path_file[i]);
